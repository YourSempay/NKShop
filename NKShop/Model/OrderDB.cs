...
namespace NKShop.Model
{
    internal class OrderDB
    {
        ...
        internal List<Statistic> SelectStatisticAll()
        {
            ...
            if (connection.OpenConnection())
            {
                var command = connection.CreateCommand($"SET @row_num = 0; SELECT (@row_num := @row_num + 1) AS Id, p.`id` AS ProductId, p.`title` AS Title, COALESCE(COUNT(DISTINCT o.`id`), 0) AS QuantityOrders, COALESCE(SUM(o.`quantity_prod`), 0) AS QuantitySells FROM `product` p LEFT JOIN `order` o ON o.`product_id` = p.`id` WHERE p.`id` <> 1 GROUP BY p.`id`, p.`title`;");
                try
                {
                    MySqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
        ...
        internal List<Statistic> SelectStatisticReady()
        {
            ...
            if (connection.OpenConnection())
            {
                var command = connection.CreateCommand($"SET @row_num = 0; SELECT (@row_num := @row_num + 1) AS Id, p.`id` AS ProductId, p.`title` AS Title, COALESCE(COUNT(DISTINCT o.`id`), 0) AS QuantityOrders, COALESCE(SUM(o.`quantity_prod`), 0) AS QuantitySells FROM `product` p LEFT JOIN `order` o ON o.`product_id` = p.`id` AND o.`is_ready` = 1 WHERE p.`id` <> 1 GROUP BY p.`id`, p.`title`;");
                try
                {
                    MySqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
        ...
        internal List<Statistic> SelectStatisticNotReady()
        {
            ...
            if (connection.OpenConnection())
            {
                var command = connection.CreateCommand($"SET @row_num = 0; SELECT (@row_num := @row_num + 1) AS Id, p.`id` AS ProductId, p.`title` AS Title, COALESCE(COUNT(DISTINCT o.`id`), 0) AS QuantityOrders, COALESCE(SUM(o.`quantity_prod`), 0) AS QuantitySells FROM `product` p LEFT JOIN `order` o ON o.`product_id` = p.`id` AND o.`is_ready` = 0 WHERE p.`id` <> 1 GROUP BY p.`id`, p.`title`;");
                try
                {
                    MySqlDataReader dr = command.ExecuteReader();
                    while (dr.Read())
